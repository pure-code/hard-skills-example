...
import {
  ColumnContainer, CandidatesList, CandidatesContainer, ColumnList,
} from './styled';
...
const Candidates = () => {
  const {
    jobs: {
      job, candidates, searchQuery, isFetchingCandidates,
    },
  } = useStore();
  const { candidateId = '' } = useParams<RouteParams>();
  const allColumns = useRef<HTMLDivElement[] | null[]>([]);
  const { handleMoveCandidate, handleOnItemMove } = useCandidateManager(allColumns.current);
      ...
      <ColumnList>
        ...
        {candidates.map(({ title: groupTitle, type, list }, columnIndex) => (
          ...
          >
            ...
            <ScrollContainer>
              <CandidatesList ref={(r) => {
                allColumns.current[columnIndex] = r;
              }}
              >
                {!isFetchingCandidates && getFilteredList(list).map((candidate) => (
                  <CandidateItem
                    key={candidate.id}
                    item={candidate}
