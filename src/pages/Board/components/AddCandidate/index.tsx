...
const fields: FieldItem[] = [
  ...
  { name: AddCandidateFieldsNames.grade, placeholder: 'Специальность', required: true },
  ...
  { type: 'textarea', name: AddCandidateFieldsNames.comment, placeholder: 'Комментарий' },
];

const AddCandidate = ({ onCreate }: AddCandidateProps) => {
  const { jobs: { addCandidate, job } } = useStore();
  const [newCandidate, setNewCandidate] = useState<Candidate>(initialCandidate(job?.id || ''));
